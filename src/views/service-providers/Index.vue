        ...
        <CCardHeader>
          <CRow>
            ...
            <CCol col="6" style="text-align: left;">
                <CButton  to="/service-providers/create" class="add" >
                    <CIcon class="c-icon" name="cil-chevron-top" /> &emsp;  إضافة مٌقدم خدمة جديد
                </CButton>
            </CCol>
        </CRow>
        </CCardHeader>
...
export default {
    ...
    return {
      options: {
        ajax: {
          ...
          url: this.$hostUrl + 'admin-dashboard/serviecs-providers',
          dataSrc: (json) => {
            return json.data
          }
        },
  ...
  methods: {
    doLoadTable (cb) {
      this.$http.getJSON(
        this.$hostUrl + 'admin-dashboard/serviecs-providers',
        function (data) {
          cb(data.data)
        }
      )
    ...
    doAlertShow (data) {
      this.$router.push({ path: `/service-providers/${data.id}/show`, params: { id: data.id } })
    },
    doAlertEdit (data) {
      this.$router.push({
        path: `/service-providers/${data.id}/edit`,
        params: { id: data.id }
      })
    },
    doAlertDelete (data, row, tr, target) {
      ...
      swal({
        ...
        if (willDelete) {
          this.$http
            .delete(
              `${this.$hostUrl}admin-dashboard/serviecs-providers/${data.id}`,
              { _method: 'delete' }
            )
            .then((response) => {
